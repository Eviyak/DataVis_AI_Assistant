...
# Загружаем ключ из секрета (настрой в .streamlit/secrets.toml)
...
if uploaded_file is not None:
    ...
    def simple_smart_plot(data, column):
        ...
        if pd.api.types.is_numeric_dtype(data[column]):
            ...
            # По умолчанию рисуем по первому числовому
    ...
    if use_ai:
        with st.spinner("AI анализирует данные..."):
            description = df[column].describe(include='all').to_string()
